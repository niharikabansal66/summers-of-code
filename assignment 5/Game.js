...
import shuffleArray from "./shuffleFunction.js";
import Puzzle from "./Puzzle";
import Timer from "./Timer";

export default function Game() {
  const [shuffledArray, setShuffledArray] = useState(shuffleArray());
  const [moves, setMoves] = useState(0); //setting moves and time to be zero
  const [time, setTime] = useState(0);
  const [timerActive, setTimerActive] = useState(false); //will start the timer after game starts
  const [win, setWin] = useState(false); //declaring win firstly as false
  ...
  return (
    <div className="h-screen flex text-gray-300 bg-gray-950">
      <div className="mx-auto mt-8">
        {win && (
          <div className="rounded-md border-l-4 border-pink-500 bg-green-100 p-2 mb-2">
